...
class MainScreenTableViewCell: UITableViewCell {
    ...
    @IBAction private func didTapPlusButton(_ sender: Any) {
        ...
        guard let approachVC = approachVC, let avc = approachVC.popoverPresentationController else { return print("No guard") }
        avc.delegate = self
        avc.sourceView = plusButton
        avc.permittedArrowDirections = [.left, .right]
        avc.sourceRect = CGRect(x: 20,
                                 y: 10,
                                 width: 0,
                                 height: 0)
        approachVC.preferredContentSize = CGSize(width: 250, height: 250)
        actionHendler?(approachVC)
    }
